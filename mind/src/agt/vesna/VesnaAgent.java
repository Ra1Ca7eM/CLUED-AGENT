...
public class VesnaAgent extends Agent{
    ...
    public void loadInitialAS( String asSrc ) throws Exception {
        ...
        Term address_term = address_unifier.get( "Address" );
        String address = termToHost( address_term );
        ...
        // Attendi handshake completo (connect() async fallisce spesso su Godot 4.6)
        boolean connected = client.connectBlocking( 15, TimeUnit.SECONDS );
        if ( !connected || !client.isOpen() ) {
            stop( "WebSocket connect failed to ws://" + address + ":" + port
                + " — avvia Godot (F5) PRIMA di gradle run" );
            return;
        }
        System.out.printf( "[%s] WebSocket connected to body%n", my_name );
    }

    private static String termToHost( Term term ) {
        if ( term.isString() )
    ...
    public void perform( String action ) {
        if ( client == null || !client.isOpen() ) {
            System.err.printf( "[%s] WS not open, cannot send: %s%n", my_name, action );
            return;
        }
        client.send( action );
    }
