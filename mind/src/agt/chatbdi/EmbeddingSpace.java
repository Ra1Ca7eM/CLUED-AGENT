...
public class EmbeddingSpace {
    ...
    /**
     * Functors (navigation / JaCaMo infrastructure) that must NOT be indexed for dialogue
     * routing. Without this filter the player's question would be routed to a navigation
     * term (e.g. "where are you?" -> ec/map_ec/ntpp). Anything starting with "map_" is also
     * ignored.
     */
    private static final java.util.Set<String> IGNORED_FUNCTORS = java.util.Set.of(
        "ntpp", "ntppi", "po", "ec", "portal_zone", "portal_entry", "portal_exit", "nav_goal",
        "door_art", "door_passage_region", "walk_poi", "find_path", "find_path_recursive",
        "path_has_door", "transit_zone", "stair_zone", "hotel_zone", "needs_center_walk",
        "poi_target", "requires_door", "same_region", "last_poi",
        "player_in", "player_entered", "player_exited", "crossing_context", "movement", "sight",
        "address", "port", "my_room", "my_name",
        "focused", "focusing", "joinedWsp", "joined", "schemes",
        // stati delle azioni: sono GUARDIE lette dai piani di dialogo, non bersagli di routing
        "digitalina_relocated", "digitalina_inspected", "pillole_wc_flushed", "pillole_wc_inspected",
        // Game Master scoring config (not dialogue routing targets)
        "weight", "pass_threshold"
    );

    /** True if the literal's functor must be excluded from the embedding space. */
    private static boolean isIgnored( Literal l ) {
        if ( l == null ) return false;
        String f = l.getFunctor();
        return f.startsWith( "map_" ) || IGNORED_FUNCTORS.contains( f );
    }
    ...
    protected void addPlan( String agName, Literal p ) {
        // Skip navigation / infrastructure functors: not relevant for dialogue routing.
        if ( isIgnored( p ) )
            return;
    ...
    private List<Literal> getAgsSubSpace( List<String> agNames, String subSpace ) {
        ...
        for ( String ag : agNames ) {
            List<Literal> domain = getAgDomain( ag );
            // A receiver can exist in the MAS but not be indexed yet (or indexing failed).
            // Ignore null domains to avoid null crashes in addAll.
            if ( domain != null )
                agsDomain.addAll( domain );
        }

        // Intersect the two sets (empty intersection if the receiver was not indexed yet)
    ...
    /** True if the agent has at least one literal in the embedding domain. */
    protected boolean isAgentIndexed( String agName ) {
        List<Literal> domain = agDomain.get( agName );
    ...
    protected Literal findNearest( List<String> ags, String subSpace, String msg ) {
        // Embed the message as a QUERY (nomic "search_query:" prefix), to match the
        // "search_document:" prefix used by Ollama.embed(Literal) when indexing the terms.
        List<Double> emb = ollama.embedQuery( msg );
